"""
ItalyVMS Slot Monitor Bot v4
- Прокси для обхода блокировки IP
- Уведомление когда токен истёк
- Команда /token для обновления токена
"""
...
SESSION_TOKEN    = "thz86n6k7a-5258469-qpw1w0heyzledagfkxvh9ay5xddwoutkm4et9cs08cy0q"
    ...
    try:
        ...
        async with httpx.AsyncClient(headers=HEADERS, proxies=proxies, timeout=30) as client:
            ...
            if r.status_code == 200:
                text = r.text.strip()
                logger.info(f"API response center={center} vtype={vtype}: '{text[:50]}'")
                ...
                logger.warning(f"403 - IP blocked")
                token_expired = True
    except Exception as e:
                ...
                try:
                    await bot.send_message(
                        ...
                        text=(
                            ...
                            "3. Скопируй токен из адресной строки (?t=ТОКЕН)\n"
                            "4. Отправь боту: /token НОВЫЙ_ТОКЕН"
                        )
                    )
                    token_alert_sent = True
                    logger.info("Token expiry alert sent to admin")
                except Exception as e:
...
async def cmd_token(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # Извлекаем токен если прислали полный URL
    ...
    token_expired = False
    save_state()
    logger.info(f"Token updated: {new_token[:20]}...")
...
async def main():
    load_state()
    logger.info(f"Starting with token: {SESSION_TOKEN[:20]}...")
